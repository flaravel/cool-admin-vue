...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		async function refresh() {
			...
			await service.base.department.list().then((res: any[]) => {
			...
			refs.value.form.open({
				...
				on: {
					submit: (data: any, { done, close }: any) => {
						service.base.department[method]({
							id: e.id,
							parentId: e.parentId,
							name: data.name,
				...
				list: [
					{
						...
						hidden:
							...
							!service.base.department._permission.add,
					...
					},
					{
						...
						hidden: !service.base.department._permission.update,
						callback: (_: any, done: Function) => {
							rowEdit(d);
							done();
						}
					},
					{
						...
						hidden: !d.parentId || !service.base.department._permission.delete,
						callback: (_: any, done: Function) => {
							rowDel(d);
							done();
						}
					},
					{
						...
						hidden: !service.base.user._permission.add,
						callback: (_: any, done: Function) => {
							emit("user-add", d);
							done();
						}
